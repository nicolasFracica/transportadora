...
def verVehiculos():
    query = """
        SELECT Vehiculo.idVehiculo, Vehiculo.modelo, Vehiculo.placa, Tipo_de_mantenimiento.nombre, Mantenimiento.idTipoMantenimiento
        FROM Vehiculo
...
def insertE(nombre, fechaNacimiento, documento, idCargo, idSucursal):
    query = """
    INSERT INTO Empleado (nombre, fechaNacimiento, documento, idCargo, idSucursal)
    values
    ('""" +nombre+ """', '"""+fechaNacimiento+"""', """+documento+""", """+idCargo+""", """+str(idSucursal)+""")
    """
    with connection.cursor() as cursor:
        cursor.execute(query)
        row = cursor.fetchall()

def insertViaje(duracion,peso, fecha, descripcion,ruta,costo,conductor,placa,material,pago):
    query1= """
    INSERT INTO Viaje (duracion,  cantidad, fecha, descripcion, idRuta) 
    values ("""+duracion+ ""","""+peso+ """,'"""+fecha+ """','"""+descripcion+ """',"""+ruta+ """)
    """
    with connection.cursor() as cursor:
        cursor.execute(query1)

        query2 = """
        select max(idViaje) from viaje 
        """
        cursor.execute(query2)
        last1 = str(cursor.fetchall()[0][0])
        query3 = """
        INSERT INTO  Factura (fecha,valor,id_pago)
        values ('"""+fecha+ """','"""+costo+ """',"""+pago+""")
        """
        cursor.execute(query3)
        ...
        """
        cursor.execute(query4)
        last2 = str(cursor.fetchall()[0][0])
        query5="""
        INSERT INTO viaje_factura (idFactura,idViaje)
        values ("""+last2+ ""","""+last1+ """) """ 
        cursor.execute(query5)

        query6 ="""
        INSERT INTO carga_viaje (idCarga,idViaje)
        values ("""+material+ """,""" +last1+""") """ 
        cursor.execute(query6)

        query9 ="""
        INSERT INTO emp_ve_vi (idEmpleado,idVehiculo,idViaje)
        values ("""+conductor+""","""+placa+""","""+last1+""") """
        cursor.execute(query9)

def insertVehiculo(modelo,placas,mantenimiento,fecha):
    with connection.cursor() as cursor:
        query1 ="""
        INSERT INTO vehiculo (modelo,placa)
        values('"""+modelo+"""','"""+placas+"""' )"""
        cursor.execute(query1)
        query3 ="""
        insert into mantenimiento (fecha,idTipoMantenimiento)
        values  ('"""+fecha+ """','"""+mantenimiento+ """')
        """
        cursor.execute(query3)
        query4 =""" select max(idVehiculo) from Vehiculo
        """
        cursor.execute(query4)
        last1 = str(cursor.fetchall()[0][0])
        query5 =""" select max(idMantenimiento) from mantenimiento
        """
        cursor.execute(query5)
        last2 = str(cursor.fetchall()[0][0])
        query6 ="""
...
def graph6():
    ...
    chartBox = ax.get_position()
    ax.set_position([chartBox.x0, chartBox.y0, chartBox.width*0.8, chartBox.height])
    ax.legend(loc='upper center', bbox_to_anchor=(3, 0.8), shadow=True, ncol=1).remove()
