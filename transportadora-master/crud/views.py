...
def agregarViaje(request):
    ...
    return render(request, 'crud/agregarViaje.html')
...
def agregarEmpleado(request):
    lugar = getSucursal()
    context = {'Sucursal': lugar}
    return render(request, 'crud/agregarEmpleado.html', context)
...
def get_data(request):
        ...
        context = {'ruta':route, 'nombre':name,'placa':plate, 'fecha':date}
        return render(request, 'crud/test.html', context)
...
def crearEmpleado(request):
    if request.method == 'POST':
        ...
        date = request.POST.get("fecha")
        sucursal = idSucursal(place)
        insertE(name, date, document, work, sucursal)
    return render(request, 'crud/avisoCE.html')
...
def crearViaje(request):
    if request.method == 'POST':
        ...
        fecha = request.POST.get("fecha")
        descripcion = request.POST.get("descripcion")
        ruta = str(idRuta(request.POST.get("ruta")))
        costo = request.POST.get("costo")
        conductor = str(getEmpleadoN(request.POST.get("conductor"))[0][0])
        placa = str(getVehiculoP(request.POST.get("placa"))[0][0])
        material = request.POST.get("material")
        pago = request.POST.get("pago")
        insertViaje(duracion,peso, fecha, descripcion,ruta,costo,conductor,placa,material, pago)
    return render(request, 'crud/avisoCVI.html')
...
def crearVehiculo(request):
    if request.method == 'POST':
        ...
        fecha = request.POST.get('fecha')
        insertVehiculo(modelo,placas,mantenimiento,fecha)
    return render(request, 'crud/avisoCVE.html')
def image(request):
    graph1()
    graph2()
    graph3()
